<?php
class EntiteController extends Zend_Controller_Action
{
	//action par défaut
	public function indexAction()
	{
		...
		$paginator = Zend_Paginator::factory($entite->fetchAll($str =array()));
	...
	public function createAction()
	{
		...
		if($this->_request->isPost())
		{
			...
			if($form->isValid($data))
			{
				...
				$entite = new Default_Model_Entite();
				$entite->setLibelle($form->getValue('libelle'));
	...
	public function editAction()
	{
		...
		if($this->getRequest()->isPost())
		{
			...
			if($form->isValid($data))
			{
				...
				//création et initialisation d'un objet Default_Model_Entite
				//qui sera enregistré dans la base de données
				$entite = new Default_Model_Entite();
				$entite->setId($form->getValue('id'));
				$entite->setLibelle($form->getValue('libelle'));
				$entite->save();
		...
		else
		{
			...
			if($id > 0)
			{
				...
				$data[] = array();
				$data['id'] = $entite->getId();
				$data['libelle'] = $entite->getLibelle();
				$form->populate($data);
