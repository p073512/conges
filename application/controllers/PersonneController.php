	...
	public function createpAction()
	{
		...
		if ($this->getRequest()->isPost())
		{
			...
			if ($form->isValid($request->getPost()))
			{
				...
				if($IsExist>0)
				{
					$this->view->error = " " . $data['Nom']." ".$data['Prenom']." existe déjé !";
				}
				else
				{
					...
					$this->view->success = $data['Nom'] . " a été bien crée !";    // <a href='".$this->_helper->url->url(array('controller' => 'personne', 'action' => 'index'), 'default', true)."'>Afficher la table Personnels </a>";
		...
		if ($this->getRequest()->isPost()) 
		{
			...
			if ($form->isValid($request->getPost())) 
			{
				...
				else
				{
					...
					$this->view->success = "La ressource ".$data['Nom']." ".$data['Prenom']. " a été bien créer !";    // <a href='".$this->_helper->url->url(array('controller' => 'personne', 'action' => 'index'), 'default', true)."'>Afficher la table Personnels </a>";
					header("Refresh:1.5;URL=".$baseurl."/personne/");              // URL dynamique
	...
	public function editAction()
	{
			...
			if($this->getRequest()->isPost())
			{
				...
				if($form->isValid($data))
				{
					...
					else
					{
						...
						$this->view->success = "La ressource ".$data['Nom']." ".$data['Prenom']. " a été bien modifié !";
		...
		elseif( $personne->getEntite()->getCs() == "0")
		{
			...
			if($this->getRequest()->isPost())
			{
				...
				//vérifie que les données répondent aux conditions des validateurs1
				if($form->isValid($data))
				{
					$isArrayEquals = true;
					// vérifie si les données ont subit une modification
