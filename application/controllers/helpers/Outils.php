   ...
   class Default_Controller_Helpers_outils extends Zend_Controller_Action_Helper_Abstract
  {
  	...
  	function jours_feries_maroc($annee) 
	{
		...
		public function setJoursFerie($annee,$maroc = false,$alsacemoselle = false)
		{
			...
			if (!$maroc) 
			{
				...
				return $this; // retourne tableau de jours fériés franÃ§ais
			}
			else 
			{
		 ...
		 *          -F : jour férié (selon le $maroc indiqué en paramétre sinon jours fériés Français)
		 ...
		 * 		  (bool) $maroc [optionnel] : $maroc Ã  true pour le csm , Ã  false pour la France
		 *		  (bool) $alsacemoselle ;
		 * 
		 * 
		 * 
		...
		public function getPeriodeDetails($annee,$dateDebut,$dateFin,$dm=false,$fm=false,$maroc=false,$alsacemoselle = false)
		{
			...
			if($maroc == false)
			{
				$iMaroc = 'France'; // indice cs france
				}
					else 
					{
						$iMaroc = 'CSM'; // indice csm
						}
	        ...
	        //récupération des jours de la période.
				  foreach ($period as $k=>$date)
				  {
				    ...
				    elseif(isset($jFerie['joursFerie'][$iMaroc][$dDate]))
				    {
				    	$conge[$dDate]['TypeJour'] ='F';
						$conge[$dDate]['LibelleFerie'] = $jFerie['joursFerie'][$iMaroc][$dDate];
				    }
					else
					{
		...
		/**
		 * 
		 * Fonction qui prend en paramÃ¨tre le tableau de détails période et retourne 
	...
	public function normaliser_date($dateDebut,$dateFin,$maroc=false,$alsacemoselle=false)
	{
		...
		return $dateNormalisee;
			...
			/**
	 * 
	 * MTA : calcul nombre jours congé  (modifié 18-06-2013) 
	...
	/**
	 * Description : 
	 * Fonction qui ajoute 12h00m00s à la date_debut si debut_midi == 1   
	 * 			 et qui ajoute 11h59m59s à la date_fin si fin_midi == 1
	 */
	...
	/**
	 * Description : 
	 * Fonction responsable de l'affichage du message succès , warning
