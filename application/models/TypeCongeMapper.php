...
class Default_Model_TypeCongeMapper
{
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_TypeConge');
		}
		return $this->_dbTable;
	}

	//sauve une nouvelle entrée dans la table
	public function save(Default_Model_TypeConge $typeconge)
	{
		//récupération dans un tableau des données de l'objet $typeconge
	...
	public function find($id, Default_Model_TypeConge $typeconge)
	{
		...
		$typeconge->setId($row->id);
		$typeconge->setCode($row->code);
		$typeconge->setLibelle($row->libelle);
	}
	...
	public function fetchAll($str)
	{
		...
		//chaque entrée est représentée par un objet Default_Model_TypeConge
		//qui est ajouté dans un tableau
		$entries = array();
		foreach($resultSet as $row)
		{
			$entry = new Default_Model_TypeConge();
			$entry->setId($row->id);
