...
function isExist(structure, indice, value, where) {
    ...
    if (congeCount % 2 == 0) congeCount = congeCount / 2;
    else congeCount = (congeCount + 1) / 2;


    if (where == 'DebutMidi' || where == 'FinMidi') {

        for (i = 0; i <= congeCount; i++) {
            if (typeof structure[i] != 'undefined') {

                if (where == 'DebutMidi') {
                    if (structure[i][0][where] == true && structure[i][0]['Date'] == value) {
                        return true;
                    }
                } else if (where == 'FinMidi') {
                    congeDays = Object.keys(structure[i])
                        .length;
                    if (congeDays % 2 == 0) congeDays = congeDays / 2;
                    else congeDays = (congeDays + 1) / 2;

                    if (structure[i][congeDays - 1][where] == true && structure[i][congeDays - 1]['Date'] == value) {
                        return true;
                    }
                }
            } else {
        ...
        for (i = 0; i <= congeCount; i++) {
            ...
            if (typeof structure[i] != 'undefined') {
                // Calcul taille structure[i] => nombre de jours par congé
                congeDays = Object.keys(structure[i])
                    .length;
            } else {
                break;
    ...
    for (var m = periode.From; m < periode.To; ++m) {
        ...
        jQuery.each(dataset.ressources, function (index, val) {
            ...
            .attr('d', function (d, i) {
						                ...
						                if (typeof dataset['ressources'][index]['conge'][indiceConge] != 'undefined' && isExist(dataset['ressources'][index]['conge'][indiceConge], 'Date', thisDate, 'DebutMidi')) {
						                    var dPathxdmf = (i * 10);
						                ...
						                if (typeof dataset['ressources'][index]['conge'][indiceConge] != 'undefined' && isExist(dataset['ressources'][index]['conge'][indiceConge], 'Date', thisDate, 'FinMidi')) {
						                ...
						                } else if (typeof dataset['ressources'][index]['conge'][indiceConge] != 'undefined' && isExist(dataset['ressources'][index]['conge'][indiceConge], 'Date', thisDate)) {
            ...
            .attr('fill', function (d, i) {
						                ...
						                if (typeof dataset['ressources'][index]['conge'][indiceConge] != 'undefined' && isExist(dataset['ressources'][index]['conge'][indiceConge], 'Date', thisDate)) {
						                    return opt.color.CP;
						                } else if (dataset['Ferie'][iCs][thisDate]) {
						                    return opt.color.FE;
						                } else if (d == 'Samedi') {
						                    ...
						                    .delay(1500)
						                     .style('font', '9px sans-serif')
						                      .text(function (d, i) {
						                                ...
						                                // si date est un congé
									                    if (typeof dataset['ressources'][index]['conge'][indiceConge] != 'undefined' && isExist(dataset['ressources'][index]['conge'][indiceConge], 'Date', thisDate)) {
									                        return 'CP'; // text = CP
									                    // si date est un férié
									                    } else if (dataset['Ferie'][iCs][thisDate]) {
									                        return 'FE';
									                    // si jour est un samedi
									                    } else if (d == 'Samedi') {
									                        return 'SA';
									                    } else if (d == 'Dimanche') {
									                        return 'DI';
									                    // si date est aujourd'hui
									                    } else if ((i + 1) == tdd && data['mm'] == tmm) {
									                        return 'AU';
									                    } else {
									                        return '';
